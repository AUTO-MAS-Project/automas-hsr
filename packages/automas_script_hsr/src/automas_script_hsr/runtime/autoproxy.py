...
from .tasks import HSR_TASK_MODULES, get_assigned_script, module_is_available
from .models import (
    CompletionWriteback,
    HSRLoginPlan,
    HSRModuleResult,
...
from .log_detect import detect_echo_of_war_completion, detect_weekly_completion
...
class HSRAutoProxyTask(TaskExecuteBase):
    ...
    def __init__(
        ...
        self.cur_user_config: Any = self.user_config[self.cur_user_uid]
    ...
    async def _open_adapter_sessions(self) -> None:
        """Open only the controller sessions effective for this script."""
        ...
        for engine in effective:
            ...
            self.runtime.sessions[engine] = session
            try:
                self.runtime.registry.track_session(engine, session)
            except Exception as setup_error:
                try:
                    await session.close()
                except Exception as cleanup_error:
                    raise ExceptionGroup(
                        f"HSR {engine} 会话注册与清理均失败",
                        [setup_error, cleanup_error],
                    ) from setup_error
                self.runtime.sessions.pop(engine, None)
                raise
        ...
        for module in HSR_TASK_MODULES:
            ...
            items.append(
                HSRRunItem(
                    ...
                    extra={"daily_eow_enabled": module_daily_eow_enabled},
                )
            )

        return items

    def _build_module_success_callback(
        self,
        *,
        module_key: str,
        module_name: str,
        engine: Literal["SRA", "M7A"],
        user_id: str,
        user_name: str,
        daily_eow_enabled: bool,
    ):
        """Build core-owned completion handling for a normalized module result."""

        if module_key == "Daily":
            return lambda result: self._queue_eow_completion_if_confirmed(
                user_id,
                user_name,
                daily_eow_enabled,
                result,
        ...
        if module_key in ("DivergentUniverse", "CurrencyWars"):
            def on_weekly_success(result: HSRNativeRunResult) -> None:
                ...
                if completed:
                    self._queue_weekly_completion(user_id, user_name, module_name)
                    return
                self._record_module_result(
                    user_id=user_id,
                    user_name=user_name,
    ...
    async def _run_queue_items(
        ...
        for phase_index, phase in enumerate(phases):
            ...
            for item_index, item in enumerate(phase_items):
                ...
                if isinstance(result, HSRRunResult) and result.status in (
                    "skipped",
                    "incomplete",
                ):
                    if item.module_key != "StartGame":
                    ...
                    self._append_log(
                        ...
                        f"{result.status}: {result.summary or result.error}"
                    )
                    continue
                ...
                if result.success:
                    ...
                    if item.module_key != "StartGame":
                        semantic_result = next(
                            (
                                recorded
                                for recorded in self.runtime.module_results
                                if recorded.user_id == item.user_id
                                and recorded.module_key == item.module_key
                            ),
                            None,
                        )
                        if (
                            semantic_result is not None
                            and semantic_result.status == "incomplete"
                        ):
                            self._append_log(
                                f"用户「{item.user_name}」模块「{item.module_name}」"
                                f"未确认业务完成：{semantic_result.reason}"
                            )
                            continue
                        self._record_module_result(
                            user_id=item.user_id,
    ...
    async def _execute_run_item(self, item: HSRRunItem) -> HSRRunResult:
        ...
        if session is None or self.runtime.registry is None:
            raise RuntimeError(f"HSR {item.script} 控制器会话当前不可用")
        group = self.runtime.registry.get_group(item.script)
        task = (
            HSRTaskDescriptor(
                key="StartGame",
                name="SRA 登录/切号",
            ...
            if item.module_key == "StartGame"
                ...
                (
                    descriptor
                    for descriptor in group.task_catalog.list_tasks()
                    if descriptor.key == item.module_key
                ),
                None,
            )
        )
        if task is None:
            raise RuntimeError(
                f"HSR {item.script} 任务目录未声明模块 {item.module_key}"
            )

        request = HSRRunRequest(
            script_id=self.script_info.script_id,
            user_id=item.user_id,
            user_name=item.user_name,
            task=task,
            ...
            script_config=self.script_config,
            user_config=item.user_cfg,
            log=self._append_log,
            extra=dict(item.extra),
        )
        result = await session.run(request)
        if result.status == "completed" and not result.completion_evidence:
            raise HSRRetryableTaskError(
                f"{item.script} 未返回明确完成证据"
            )
        return result
    ...
    async def main_task(self):
        ...
        await self._open_adapter_sessions()
        ...
        for attempt in range(1, retry_limit + 1):
            ...
            daily_completed_with_evidence = any(
                result.user_id == uid
                and result.module_key in MODULE_KEYS_BY_PHASE["daily"]
                and result.status == "completed"
                for result in self.runtime.module_results
            )
            if daily_was_attempted and daily_completed_with_evidence:
    ...
    async def final_task(self):
        ...
        sessions = list(self.runtime.sessions.items())
        for engine, session in reversed(sessions):
            try:
                await session.close()
            except Exception as e:  # noqa: BLE001
                ...
                if self.runtime.sessions.get(engine) is session:
                    self.runtime.sessions.pop(engine, None)
        ...
        for path in self.temp_files:
            try:
                ...
                logger.warning(f"清理 HSR 临时配置失败：{path}")
